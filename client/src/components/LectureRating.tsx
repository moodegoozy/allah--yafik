        ...
        <AnimatePresence mode="wait">
          {!submitted ? (
            <motion.div key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              ...
              <p className="text-white/40 text-xs mb-4">رأيك يساعدنا على تحسين المحتوى</p>
                ...
                {showForm && (
                  ...
                  >
                        ...
                        <button
                          ...
                          className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all ${
                            selectedFeedback.includes(opt)
                              ? "text-white"
                              : "glass-card text-white/40 border border-white/8 hover:text-white/70"
                          }`}
                    ...
                    <textarea
                      ...
                      className="w-full bg-white/4 border border-white/8 rounded-xl p-3 text-white/80 text-sm placeholder-white/25 resize-none focus:outline-none focus:border-white/20 mb-4"
                      rows={2}
                    />
                    ...
                    <button
                      ...
                      style={{ background: `linear-gradient(135deg, ${color}, #00D4AA)`, color: "#060B18" }}
                    >
            ...
            >
              <div className="w-12 h-12 rounded-2xl bg-[#10B981]/20 flex items-center justify-center mx-auto mb-3">
                <ThumbsUp className="w-6 h-6 text-[#10B981]" />
              </div>
              <h4 className="text-white font-bold mb-1">شكراً على تقييمك!</h4>
              <div className="flex justify-center gap-1 mb-2">
                {[1, 2, 3, 4, 5].map(s => (
                  <Star key={s} className="w-4 h-4" style={{ color: s <= rating ? "#F59E0B" : "rgba(255,255,255,0.1)", fill: s <= rating ? "#F59E0B" : "none" }} />
                ))}
              </div>
              <p className="text-white/40 text-xs">تقييمك يساعدنا على تحسين البرنامج</p>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
        ...
        <div className="space-y-3">
          {relatedLectures.map((lec, i) => (
            <motion.button
              ...
              className="w-full flex items-center gap-3 p-3 rounded-xl glass-card border border-white/5 hover:border-white/15 transition-all group text-right"
            >
              <div
              ...
              <div className="flex-1 min-w-0">
                <h5 className="text-white/80 font-bold text-xs line-clamp-1 group-hover:text-white transition-colors">{lec.title}</h5>
                <p className="text-white/30 text-xs">{lec.speaker} · {lec.duration}</p>
              </div>
