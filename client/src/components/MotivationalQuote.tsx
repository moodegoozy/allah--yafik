...
export default function MotivationalQuote() {
  ...
  return (
    <div className="glass-card p-5 border border-[#8B5CF6]/20 relative overflow-hidden">
      <div className="absolute inset-0 bg-gradient-to-br from-[#8B5CF6]/5 to-transparent" />
      <div className="relative z-10">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <Sparkles className="w-4 h-4 text-[#8B5CF6]" />
            <span className="text-[#8B5CF6] text-xs font-medium">اقتباس اليوم</span>
          </div>
          <button
            onClick={nextQuote}
            className="w-7 h-7 rounded-lg bg-white/5 flex items-center justify-center text-white/30 hover:text-[#8B5CF6] transition-colors"
          >
            <RefreshCw className="w-3.5 h-3.5" />
          </button>
        </div>
        <p
          className="text-white/80 text-sm leading-relaxed mb-2 transition-opacity duration-300"
          style={{ opacity: fade ? 1 : 0 }}
        >
          "{quote.text}"
        </p>
        <p className="text-white/30 text-xs">— {quote.author}</p>
      </div>
    </div>
  );
}
