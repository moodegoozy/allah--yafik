...
import {
  Sparkles, Eye, EyeOff, User, Lock, Phone,
  Mail, CheckCircle2, ArrowLeft, Shield, Heart
} from "lucide-react";
...
export default function Login() {
  ...
  const [registerForm, setRegisterForm] = useState({
    ...
    addictionType: "", soberDays: "", agreeTerms: false,
  });
  const [forgotPhone, setForgotPhone] = useState("");

  const addictionTypes = [
    "مخدرات", "تدخين", "كحول", "إدمان رقمي / ألعاب",
    "إدمان طعام", "إدمان عمل", "أخرى"
  ];
  ...
  const handleLogin = () => {
    ...
    setTimeout(() => {
      ...
      const user = users.find((u: any) => u.phone === loginForm.phone && u.password === loginForm.password);
      if (user) {
        ...
        // Demo login
        const demoUser = { name: "أحمد محمد", phone: loginForm.phone, soberDays: 47, addictionType: "تدخين" };
        localStorage.setItem("allah_yafik_current_user", JSON.stringify(demoUser));
        toast.success("مرحباً بك في برنامج الله يعافيك!");
        navigate("/dashboard");
      }
      setLoading(false);
    }, 1200);
  ...
  const handleRegister = () => {
    ...
    setTimeout(() => {
      ...
      localStorage.setItem("allah_yafik_current_user", JSON.stringify(newUser));
  ...
  const handleForgot = () => {
    if (!forgotPhone) { toast.error("يرجى إدخال رقم الجوال"); return; }
    ...
    <div className="min-h-screen bg-[#060B18] flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10">
          ...
          {mode === "login" && (
            <motion.div key="login" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
              <div className="glass-card p-7 border border-white/10">
                ...
                <p className="text-white/40 text-sm mb-6">أهلاً بعودتك في رحلة التعافي</p>

                <div className="space-y-4">
                  <div>
                    <label className="text-white/50 text-xs font-bold mb-1.5 block">رقم الجوال</label>
                  ...
                  <div>
                    <label className="text-white/50 text-xs font-bold mb-1.5 block">كلمة المرور</label>
                    <div className="relative">
                      ...
                      <input
                        ...
                        dir="ltr"
                      />
                      <button onClick={() => setShowPass(!showPass)} className="absolute left-3 top-1/2 -translate-y-1/2 text-white/30 hover:text-white/60">
                        {showPass ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                      </button>
                    </div>
                  </div>
                  <div className="flex justify-end">
                    <button onClick={() => setMode("forgot")} className="text-[#00D4AA] text-xs hover:text-[#00D4AA]/70 transition-colors">
                      نسيت كلمة المرور؟
                    </button>
                  </div>
                  <button
                  ...
                  >
                    {loading ? <div className="w-5 h-5 border-2 border-[#060B18]/30 border-t-[#060B18] rounded-full animate-spin" /> : "دخول"}
                  </button>
                </div>

                <div className="mt-5 pt-5 border-t border-white/5 text-center">
                  <span className="text-white/40 text-sm">ليس لديك حساب؟ </span>
                  <button onClick={() => setMode("register")} className="text-[#00D4AA] font-bold text-sm hover:text-[#00D4AA]/70">
                    إنشاء حساب جديد
                  </button>
                </div>
              </div>
              ...
              {/* Guest Access */}
              <button
                onClick={() => { localStorage.setItem("allah_yafik_current_user", JSON.stringify({ name: "زائر", phone: "", soberDays: 0 })); navigate("/dashboard"); }}
            ...
            <motion.div key="register" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
              <div className="glass-card p-7 border border-white/10 max-h-[75vh] overflow-y-auto">
                <div className="flex items-center gap-3 mb-5">
                  <button onClick={() => setMode("login")} className="text-white/40 hover:text-white transition-colors">
                    <ArrowLeft className="w-5 h-5" />
                  </button>
                  <div>
                    <h2 className="text-white font-black text-xl">إنشاء حساب</h2>
                    <p className="text-white/40 text-xs">ابدأ رحلة التعافي اليوم</p>
                  </div>
                </div>
                ...
                <div className="space-y-3.5">
                  ...
                  <div>
                    <label className="text-white/50 text-xs font-bold mb-1.5 block">البريد الإلكتروني</label>
                    <div className="relative">
                  ...
                  <div>
                    <label className="text-white/50 text-xs font-bold mb-1.5 block">نوع الإدمان (اختياري)</label>
                    <select
                      value={registerForm.addictionType}
                      onChange={e => setRegisterForm(p => ({ ...p, addictionType: e.target.value }))}
                    ...
                    >
                      ...
                      {addictionTypes.map(t => <option key={t} value={t}>{t}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="text-white/50 text-xs font-bold mb-1.5 block">عدد أيام التعافي (إن وجد)</label>
                    <input
                      type="number"
                      value={registerForm.soberDays}
                      onChange={e => setRegisterForm(p => ({ ...p, soberDays: e.target.value }))}
                      placeholder="0"
                      min="0"
                  ...
                  <div>
                    <label className="text-white/50 text-xs font-bold mb-1.5 block">تأكيد كلمة المرور *</label>
                    <input
                      type="password"
                  ...
                  <div className="flex items-start gap-3 p-3 rounded-xl bg-white/3 border border-white/8">
                    <button
                      onClick={() => setRegisterForm(p => ({ ...p, agreeTerms: !p.agreeTerms }))}
                    ...
                    >
                      {registerForm.agreeTerms && <CheckCircle2 className="w-3 h-3 text-[#060B18]" />}
                    </button>
                    <p className="text-white/40 text-xs leading-relaxed">
                      أوافق على شروط الاستخدام وسياسة الخصوصية. بياناتي محفوظة بسرية تامة ولن تُشارك مع أي جهة.
                    </p>
                  </div>
                  ...
                  <button
                    ...
                    style={{ background: "linear-gradient(135deg, #00D4AA, #0EA5E9)" }}
                  >
                    {loading ? <div className="w-5 h-5 border-2 border-[#060B18]/30 border-t-[#060B18] rounded-full animate-spin" /> : "إنشاء الحساب"}
                  </button>
                </div>
              </div>
            </motion.div>
            ...
            <motion.div key="forgot" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
              <div className="glass-card p-7 border border-white/10">
                <div className="flex items-center gap-3 mb-5">
                  ...
                  <div>
                    <h2 className="text-white font-black text-xl">استعادة كلمة المرور</h2>
                    <p className="text-white/40 text-xs">سنرسل لك رمز التحقق</p>
                  </div>
                </div>
                <div className="space-y-4">
                  <div>
                    <label className="text-white/50 text-xs font-bold mb-1.5 block">رقم الجوال المسجل</label>
                    <div className="relative">
                      <Phone className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-white/30" />
                      <input
                        value={forgotPhone}
        ...
        <div className="mt-5 flex items-center justify-center gap-2 text-white/25 text-xs">
          ...
          <a href={`tel:${CONTACT_PHONE}`} className="text-[#00D4AA]/50 hover:text-[#00D4AA]">{CONTACT_PHONE}</a>
        </div>
      </div>
    </div>
  );
