      ...
      <div className="page-content overflow-y-auto">
        ...
        {showNewPost && (
          ...
          >
            ...
            <div className="flex gap-2 mt-2">
              <button
                onClick={handlePost}
                className="flex-1 py-2 rounded-xl font-bold text-xs text-[#060B18]"
                style={{
              ...
              <button
                ...
                className="px-4 py-2 rounded-xl glass-card border border-white/10 text-white/50 text-xs font-bold"
              >
                إلغاء
              </button>
            </div>
        ...
        <div className="px-4 mt-4">
          ...
          {groups.length > 0 ? (
            <div className="flex gap-3 overflow-x-auto pb-1">
              {groups.map((group, idx) => (
                ...
                >
                  ...
                  <div className="text-white font-black text-xs mb-0.5">
                    {group.name}
                  </div>
            ...
            <div className="text-center py-6 rounded-2xl glass-card border border-white/8">
              <Users className="w-8 h-8 text-white/10 mx-auto mb-2" />
              <p className="text-white/30 text-xs">لا توجد مجموعات بعد</p>
            </div>
          )}
        </div>
        ...
        <div className="px-4 mt-4 space-y-4 mb-4">
          ...
          {postsList.filter(
            ...
            <div className="text-center py-12">
              ...
              <p className="text-white/30 text-sm">لا توجد منشورات بعد</p>
              <p className="text-white/20 text-xs mt-1">كن أول من يشارك!</p>
            </div>
          )}
          {postsList
            .filter(
            ...
            .map((post, idx) => (
              ...
              >
                ...
                <div className="flex items-center gap-2 mb-3">
                  <div
                    className={cn(
                      "w-8 h-8 rounded-full flex items-center justify-center font-black text-white text-xs bg-gradient-to-br",
                      post.avatarColor
                    )}
                  >
                    {post.avatar}
                ...
                <div className="flex items-center gap-3">
                  <button
                    ...
                    className={cn(
                      ...
                      post.liked ? "text-[#EF4444]" : "text-white/30"
                    )}
                  >
                    <Heart
                      className={cn(
                        "w-3.5 h-3.5",
                        post.liked && "fill-[#EF4444]"
                      )}
                    />
                    {post.likes}
                  </button>
